...
- 10. When writing SQL queries in repositories that JOIN other tables, always use full descriptive column aliases — never short prefixes. The alias must clearly indicate which table it comes from and what the column represents. For example, if joining `repair_status` for a current status, use `current_status_code`, `current_status_name` etc. If joining `users` for a requester, use `requester_email`, `requester_name` etc. Never use short prefixes like `cs_`, `req_`, `p_`, `rs_`. This ensures `search` and `searchTerm` field names are self-explanatory and maintainable.
- 11. All `GetList*` repository methods must wrap their JOIN query in a subquery (`SELECT * FROM (...) base WHERE ...`) so that the WHERE clause (used by `QueryBuilder`) operates on flat, unambiguous column names. This prevents column name conflicts when joined tables share column names (e.g. `name`, `code`, `deleted`) with the main table.
- 12. Do not touch anything in drizzle schema
- 13. When you write somethings in `.md` file and does not belong to `docs`. Please put it in `etc` instead of root.
